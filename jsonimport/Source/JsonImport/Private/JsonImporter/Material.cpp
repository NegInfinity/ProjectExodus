...
void JsonImporter::importMaterial(JsonObjPtr obj, int32 matId){
	...
	//Gamma correction.
	auto specularColor = applyGamma(jsonMat.specularColor);
	auto color = applyGamma(jsonMat.color);

	UMaterial *existingMaterial = nullptr;
	UPackage *matPackage = createPackage(
		jsonMat.name, jsonMat.path, assetRootPath, FString("Material"), 
		&sanitizedPackageName, &sanitizedMatName, &existingMaterial);
	if (existingMaterial){
		matIdMap.Add(jsonMat.id, existingMaterial->GetPathName());
		UE_LOG(JsonLog, Log, TEXT("Found existing material: %s (package %s)"), *sanitizedMatName, *sanitizedPackageName);
		return;
	}

	bool isTransparentQueue = (jsonMat.renderQueue >= 3000) && (jsonMat.renderQueue < 4000);
	bool isAlphaTestQueue = (jsonMat.renderQueue >= 2450) && (jsonMat.renderQueue < 3000);
	bool isGeomQueue = (!isTransparentQueue && !isAlphaTestQueue) || 
		((jsonMat.renderQueue >= 2000) && (jsonMat.renderQueue < 2450));
	...
	auto albedoSource = createMaterialInputMultiply(material, jsonMat.mainTexture, &color, material->BaseColor, 
	...
	if (jsonMat.useNormalMap){
		...
		createMaterialInput(material, jsonMat.normalMapTex, nullptr, material->Normal, true, TEXT("Normal"));
	}
	...
	if (jsonMat.hasSpecular){
		createMaterialInput(material, jsonMat.specularTex, &specularColor, material->Specular, false, 
	...
	createMaterialInput(material, jsonMat.occlusionTex, nullptr, material->AmbientOcclusion, false, TEXT("Ambient Occlusion"));
	...
	if (jsonMat.hasEmission){
		...
		createMaterialInputMultiply(material, jsonMat.emissionTex, jsonMat.hasEmissionColor ? &jsonMat.emissionColor: 0, material->EmissiveColor, 
	...
	if (material){
		matIdMap.Add(jsonMat.id, material->GetPathName());
